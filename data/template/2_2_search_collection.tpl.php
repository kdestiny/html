<?php if(!defined('IN_DISCUZ')) exit('Access Denied'); hookscriptoutput('collection');
0
|| checktplrefresh('./template/default/search/collection.htm', './template/default/search/header.htm', 1551626504, '2', './data/template/2_2_search_collection.tpl.php', './template/h2o_2018_qing', 'search/collection')
|| checktplrefresh('./template/default/search/collection.htm', './template/default/search/pubsearch.htm', 1551626504, '2', './data/template/2_2_search_collection.tpl.php', './template/h2o_2018_qing', 'search/collection')
|| checktplrefresh('./template/default/search/collection.htm', './template/default/search/collection_list.htm', 1551626504, '2', './data/template/2_2_search_collection.tpl.php', './template/h2o_2018_qing', 'search/collection')
|| checktplrefresh('./template/default/search/collection.htm', './template/default/search/footer.htm', 1551626504, '2', './data/template/2_2_search_collection.tpl.php', './template/h2o_2018_qing', 'search/collection')
|| checktplrefresh('./template/default/search/collection.htm', './template/h2o_2018_qing/common/header_common.htm', 1551626504, '2', './data/template/2_2_search_collection.tpl.php', './template/h2o_2018_qing', 'search/collection')
;?>
...
<?php if(empty($_GET['diy'])) { $_GET['diy'] = '';?><?php } if(!isset($topic)) { $topic = array();?><?php } $todayposts = DB::result_first("SELECT sum(todayposts) FROM ".DB::table('forum_forum')." WHERE status=1");?>    <?php $yesterdayposts = DB::result_first("SELECT sum(yesterdayposts) FROM ".DB::table('forum_forum')." WHERE status=1");?></head>
...
<?php } ?><?php if(!empty($_G['setting']['pluginhooks']['collection_top'])) echo $_G['setting']['pluginhooks']['collection_top'];?>
...
</div><?php echo adshow("search/y mtw");?><?php if(empty($collectionlist)) { ?>
<p class="emp xs2 xg2">对不起，没有找到匹配结果。</p>
<?php } else { ?>
<div class="slst pbm bbda cl"><?php if(is_array($collectionlist)) foreach($collectionlist as $key => $value) { ?><dl class="xld xld_a z" style="width: 350px;">
<dt><a href="forum.php?mod=collection&amp;action=view&amp;ctid=<?php echo $value['ctid'];?>" target="_blank"><?php echo $value['name'];?></a></dt>
<dd>主题: <?php echo $value['threadnum'];?>, 评论: <?php echo $value['commentnum'];?>, 订阅: <?php echo $value['follownum'];?>, 最后更新: <?php echo $value['lastupdate'];?></dd>
<dd><?php echo $value['desc'];?>&nbsp;</dd>
</dl>
<?php } ?>
</div>
<?php if(!empty($multipage)) { ?><br /><div class="pgs cl mbm"><?php echo $multipage;?></div><?php } } ?>
</div>
<?php } ?>

</div>
<?php if(!empty($_G['setting']['pluginhooks']['collection_bottom'])) echo $_G['setting']['pluginhooks']['collection_bottom'];?><?php $focusid = getfocus_rand($_G[basescript]);?><?php if($focusid !== null) { $focus = $_G['cache']['focus']['data'][$focusid];?><div class="focus" id="focus">
<div class="bm">
<div class="bm_h cl">
