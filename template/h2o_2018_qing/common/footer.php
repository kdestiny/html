...
<div id="ft" class="cl">
	...
	<div class="wp">
		...
		<div id="frt" class="z">
			<p>Powered by <strong><a href="http://www.discuz.net" target="_blank">Discuz!</a></strong> <em>$_G['setting']['version']</em>&nbsp;&nbsp;&copy; 2001-2013 <a href="http://www.comsenz.com" target="_blank">Comsenz Inc.</a><!--{if !empty($_G['setting']['boardlicensed'])}--> <a href="http://license.comsenz.com/?pid=1&host=$_SERVER[HTTP_HOST]" target="_blank">Licensed</a><!--{/if}--></p>
		</div>
		<!--{eval updatesession();}-->
...
<div id="h2o-scroll">
	<a id="h2o-scrollTop" class="scroll" title="{lang scrolltop}"></a>
	<!--{if $_G[fid] && $_G['mod'] == 'viewthread'}-->
		<a href="forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]&extra=$_GET[extra]&page=$page{if $_GET[from]}&from=$_GET[from]{/if}" onclick="showWindow('reply', this.href)" class="fast" title="{lang fastreply}"></a></span>
	<!--{/if}-->
	<!--{if $_G[fid]}-->
		<!--{if $_G['mod'] == 'viewthread'}-->
		<a href="forum.php?mod=redirect&goto=nextoldset&tid=$_G[tid]" class="last" title="上一主题" ></a>
		<a href="forum.php?mod=redirect&goto=nextnewset&tid=$_G[tid]" class="next" title="下一主题" ></a>
		<a class="return" href="forum.php?mod=forumdisplay&fid=$_G[fid]" hidefocus="true" title="{lang return_list}"></a>
		<!--{else}-->
		<a class="return" href="forum.php" hidefocus="true" title="{lang return_forum}"></a>
		<!--{/if}-->
	<!--{/if}-->
</div>
