...
<div id="threadlist" class="tl bm bmw h2o_ab_bmw"{if $_G['uid']} style="position: relative;"{/if}>
					...
					<tbody>
						<tr>
							<th style="border-top: none;">
								<!--{if $_G['forum']['ismoderator'] && !$_GET['archiveid']}--><div class="o">&nbsp;</div><!--{/if}-->
								<div class="h2o_post_avatar"><a href="home.php?mod=space&uid=$announcement[authorid]" target="_blank"><img src="$_G['style']['styleimgdir']/h2o_ann_ico.png" /></a></div>
								<div style="padding-left: 80px;">
									<div class="h2o_post_list_tit">[{lang announcement}] <!--{if empty($announcement['type'])}--><a href="forum.php?mod=announcement&id=$announcement[id]#$announcement[id]" target="_blank">$announcement[subject]</a><!--{else}--><a href="$announcement[message]" target="_blank">$announcement[subject]</a><!--{/if}--></strong>
								</div>
									<div class="h2o_post_list_other">
										<div class="author" style="float: left;margin-right: 15px;">
										<a href="home.php?mod=space&uid=$announcement[authorid]" c="1">$announcement[author]</a></div>
										<span class="time">发布于&nbsp;&nbsp;&nbsp;$announcement[starttime]</span>
									</div>
								</div>
							</th>
						</tr>
				...
				<!--{if $_G['forum_threadcount']}-->
					<!--{if empty($_G['forum']['picstyle']) || $_G['cookie']['forumdefstyle']}-->
						<!--{loop $_G['forum_threadlist'] $key $thread}-->	
						    <!--{eval $threadtable =  DB::fetch_all('SELECT * FROM '.DB::table('forum_attachment').' WHERE tid = '. $thread['tid'].' LIMIT  0 ,'. 4);}--> 
							<!--{if $_G[setting][forumseparator] == 1 && $separatepos == $key + 1}-->
								<tbody id="separatorline">
									<tr class="ts" style="background: #f9f9f9;height: 40px;line-height: 40px;">
										<th>
											<!--{if $_G['forum']['ismoderator'] && !$_GET['archiveid']}--><div class="o">&nbsp;</div><!--{/if}-->
											<div style="text-align: center;"><!--{if empty($_G['forum']['picstyle']) && $_GET['orderby'] == 'lastpost' && !$_GET['filter']}--><a href="javascript:;" onclick="checkForumnew_btn('{$_G['fid']}')" title="{lang showupgrade}" class="forumrefresh" style="font-size: 16px">{lang forum_thread}</a><!--{else}-->&nbsp;<!--{/if}--></div>
										</th>
									</tr>
								</tbody>
							...
							<tbody id="$thread[id]"{if $_G['hiddenexists'] && $thread['hidden']} style='display:none'{/if}>
								<tr>
									<th>
										...
										<div {if $_G['uid']}style="padding-left: 80px;"{else}style="padding-left: 60px;"{/if}>
										...
										<!--{/if}-->
												...
												<!--{if in_array($thread['displayorder'], array(1, 2, 3, 4))}-->
											        <img src="$_G['style'][styleimgdir]/pin_$thread[displayorder].gif" alt="$_G[setting][threadsticky][3-$thread[displayorder]]" />
										        <!--{/if}-->
												<!--{if $thread[folder] == 'lock'}-->
											        <img src="$_G['style'][styleimgdir]/folder_lock.gif" />
										        <!--{elseif $thread['special'] == 1}-->
											        <img src="$_G['style'][styleimgdir]/pollsmall.gif" alt="{lang thread_poll}" />
									        	<!--{elseif $thread['special'] == 2}-->
											        <img src="$_G['style'][styleimgdir]/tradesmall.gif" alt="{lang thread_trade}" />
										        <!--{elseif $thread['special'] == 3}-->
											        <img src="$_G['style'][styleimgdir]/rewardsmall.gif" alt="{lang thread_reward}" />
										        <!--{elseif $thread['special'] == 4}-->
											        <img src="$_G['style'][styleimgdir]/activitysmall.gif" alt="{lang thread_activity}" />
										        <!--{elseif $thread['special'] == 5}-->
											        <img src="$_G['style'][styleimgdir]/debatesmall.gif" alt="{lang thread_debate}" />
										        <!--{/if}-->
												</a>
											...
											<div class="h2o_post_list_other cl">
												...
												<div class="z">
												    <span class="author"><!--{if $thread['authorid'] && $thread['author']}-->
											    ...
											    <span class="time z"><span style="margin-right: 10px;">|</span><!--{if $thread['lastposter']}--><a href="{if $thread[digest] != -2}home.php?mod=space&username=$thread[lastposterenc]{else}forum.php?mod=viewthread&tid=$thread[tid]&page={echo max(1, $thread[pages]);}{/if}" c="1"    >$thread[lastposter]</a><!--{else}-->$_G[setting][anonymoustext]<!--{/if}-->			<span>@</span><span>$thread[lastpost]</span></span>
